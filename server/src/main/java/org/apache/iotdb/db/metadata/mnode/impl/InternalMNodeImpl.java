...
public class InternalMNodeImpl extends MNodeBase implements InternalMNode {
  ...
  public InternalMNodeImpl(MNode parent, String name) {
    super(parent, name);
  ...
  public void deleteChild(String name) throws DeleteFailedException {
    if (children.containsKey(name)) {
      ...
      if (children.get(name) instanceof LeafMNodeImpl) {
        writeLock = lock.writeLock();
      } else {
        // otherwise, we only need to acquire the write lock of its child node.
        writeLock = ((InternalMNodeImpl) children.get(name)).lock.writeLock();
      }
  ...
  public void readLock() {
    InternalMNodeImpl node = this;
    while (node != null) {
      node.lock.readLock().lock();
      node = (InternalMNodeImpl) node.parent;
    }
  }
